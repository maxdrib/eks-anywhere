...
mocks: ## Generate mocks
	...
	${GOPATH}/bin/mockgen -destination=pkg/providers/tinkerbell/mocks/client.go --build_flags=--mod=mod -package=mocks "github.com/aws/eks-anywhere/pkg/providers/tinkerbell" ProviderKubectlClient
	...
	${GOPATH}/bin/mockgen -destination=pkg/networking/cilium/mocks/clients.go --build_flags=--mod=mod -package=mocks -source "pkg/networking/cilium/client.go"
	${GOPATH}/bin/mockgen -destination=pkg/networking/cilium/mocks/helm.go --build_flags=--mod=mod -package=mocks -source "pkg/networking/cilium/templater.go"
	${GOPATH}/bin/mockgen -destination=pkg/networking/cilium/mocks/upgrader.go --build_flags=--mod=mod -package=mocks -source "pkg/networking/cilium/upgrader.go"
	${GOPATH}/bin/mockgen -destination=pkg/networking/kindnetd/mocks/client.go --build_flags=--mod=mod -package=mocks -source "pkg/networking/kindnetd/upgrader.go"
	${GOPATH}/bin/mockgen -destination=pkg/networking/cilium/mocks/cilium.go --build_flags=--mod=mod -package=mocks -source "pkg/networking/cilium/cilium.go"
